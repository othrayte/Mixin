 ...
 * @property {Album|Playlist|null}  context  Get or set the context.
 * @property {boolean}              playing  Get or set playing state.
 */
Player.prototype.play = function(track, context) {
    //console.log("Player#play", track, context);
    if (context)
        this.context = context;
    m.player.play(track, context);
    //console.log(m.player.context)
};

/**
 * Update player view to match the underlying player object,
 * check if it's playing from the same context etc.
 * @ignore
 * @this {Player}
...
/**
 * List view.
 * @param    {Collection=} collection  The collection which is to be displayed.
 * @param    {function=}   getItem     A function that returns a view for a single item.
 * @param    {function=}   filter      A function that filters the collection.
 * @property {number}      itemHeight  Get or set the height of the individual items. Note that your CSS must ensure that it looks correct.
 * @property {Collection}  collection  Get or set the collection which is displayed by the list view.
 * @constructor
 */
function List(collection, getItem, filter) {
    var node = this.node = div.cloneNode(); // The node that scrolls and stuff
    var list = this;
    this._length = 0;
    this._age = 0;
    this._wrapperNode = this.node.appendChild(div.cloneNode());
    this._getItemView = getItem || Track.withFields;
    this._filter = filter || listDefaultFilter;
...
function updateList(list, index) {
    listUpdateCollection.call(list, list._collection);
    //listClean.call(list, force);
    var item;

    if (true == (index in list._itemViews)) {
        item = list._itemViews[index];
        item.node.parentNode.removeChild(item.node);
        delete list._itemViews[index];
    }

    item = list._getItemView(list._collection.get(list._items[index]));

    item._index = list._items[index];
    if (m.player.track && m.player.track.uri === item.track.uri && item._index === m.player.index) {
        item.playing = true;
        if (!m.player.playing)
            item.paused = true;
    }
    if (index === list._selection)
        item.selected = true;
    list._itemViews[index] = item;

    item.node.dataset["itemindex"] = list._items[index];
    item.node.dataset["viewindex"] = index;
    $(item.node).css("height: 50px");

    item.node.style.webkitTransform = "translateY(" + index * list._itemHeight + "px)";
    item._age = list._age;
    list._wrapperNode.appendChild(item.node);
}
...
function listUpdateHTML(index) {
    ...
    for (var i = startItem, l = list._length; i <= endItem && i < l; ++i) {
        ...
        if (i == index && index < list._itemViews.length) {
            item = list._itemViews(index);
            item.node.parentNode.removeChild(item.node);
        ...
        $(item.node).css("background-color: red;");
        $("#left").append("(@)");
...
function Track(track, fields) {
    ...
    if (fields & Track.FIELD.POPULARITY) {
        ...
        field.innerHTML = l.format("<span class=\"sp-popularity-indicator\"><span class=\"sp-popularity-indicator-value\" style=\"width:{0}%\"></span></span>", track.data.popularity);
        this.node.appendChild(field);
    }
...
var DEFAULT_FIELDS = Track.FIELD.STAR     | Track.FIELD.SHARE
                   | Track.FIELD.NAME     | Track.FIELD.ARTIST
                   | Track.FIELD.DURATION | Track.FIELD.ALBUM;
 ...
 * @param {string=} uri     A URI which, if provided, will be used for an anchor wrapping the image.
 * @param {string=} title   A string which, if provided, will be used as the title for the anchor wrapping the image.
 * @property {Node} node    The node of the view.
 */
function _Image(source, uri, title) {
    var image = this;
    ...
    if (title) {
        image.node.title = title;
    }

    image.node.classList.add("sp-image");

    if (0 === source.indexOf("spotify:mosaic:")) {
        var promises = [];
        var srcs = map(function(src) { return "spotify:image:" + src; },
            source.slice(15).split(/[,;:]/));
        var domNodeInserted = new p.Promise();
        var pr;
        image.node.addEventListener("DOMNodeInsertedIntoDocument", function domNodeListener() {
            domNodeInserted.resolve();
            image.node.removeEventListener("DOMNodeInsertedIntoDocument", domNodeListener);
        });
        promises.push(domNodeInserted);
        srcs.forEach(function(src, index){
            var img = new Image();
            var pr = new p.Promise();
            img.src = src;
            if (img.complete) {
                pr.resolve(img);
            } else {
        ...
        pr = p.join.apply(null, promises);
        pr.always(function(img_) {
            var img, w, h, n, id, context, i, x, y;

            w = image.node.offsetWidth * 2 || 300;
            h = w;
            ...
            for (i = 0; i < arguments.length && i < n * n; i++) {
                img = arguments[i];
                x = i % n;
                y = Math.floor(i / n);
                h = image.node.offsetWidth / img.width * img.height * 2 || 300;
                context.drawImage(img, x * w / n, y * h / n, w / n, h / n);
            };

            image.node.style.backgroundImage = "-webkit-canvas(cover-"+ id +")";
        });
        return;
    }
 ...
 * @deprecated
 * @param {Album|Playlist} collection
 */
